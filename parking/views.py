...
class parking_lots_view(APIView):
    ...
    def get(self, request):
        ...
        serializer = parking_lot_serializer(lots, many=True) #convert to JSON
        return Response(serializer.data) #Send back in http response.

class parking_event_view(APIView):
    #is Authenicated ensure that the user logged in
    permission_classes = [IsAuthenticated]
    ...
    def post(self, request, action):
        ...
        try:
            if action == 'park':
                mark_parked(request.user.user_id, lot_id)
            elif action == 'leave':
                mark_left(request.user.user_id, lot_id)
...
class parking_action_view(APIView):
    #is Authenicated ensure that the user logged in
    permission_classes = [IsAuthenticated]
    ...
    def post(self, request, action):
        ...
        if action == 'park':
            ...
            has_active_parking = parking_event.objects.filter(
